...
CONTRAST = float(os.environ.get("ASCII_CONTRAST", 0.92))
BRIGHTNESS = 1.0
GAMMA = float(os.environ.get("ASCII_GAMMA", 1.65))          # >1 brightens mids -> face lands in sparser chars
SHARPEN = False
WHITE_FLOOR = float(os.environ.get("ASCII_WHITE_FLOOR", 0.60))    # luminance above this is forced to blank (space)
...
cursor_offset = PAD + int((len(whoami_str) + 1) * 7.8)
